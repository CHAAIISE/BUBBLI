...
contract MoodNFT is ERC721, Ownable, ReentrancyGuard {
    ...
    function mintNFT(Mood mood) public nonReentrant returns (uint256) {
        require(!hasMinted[msg.sender], "User has already minted an NFT");

        uint256 newItemId = tokenCounter;
        tokenCounter++;

        hasMinted[msg.sender] = true;
        userTokenId[msg.sender] = newItemId;
    ...
    function changeMood(Mood newMood) public nonReentrant {
        ...
        require(
            oldMood != newMood,
            "New mood must be different from the current mood"
        );
